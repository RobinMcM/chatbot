...
export default function ViewChatsPanel({
  apiBase,
  chatMode,
  sessionId,
  linkedEmail,
  ...
  }, [chatMode, sessionId, linkedEmail, refreshTrigger, apiBase]);

  const handleSelectConversation = async (c) => {
    const clientId = c.client_id;
    const selectedConversationId = c.conversation_id;
    if (!clientId || !selectedConversationId || !onLoadConversation) return;
    setLoadingConversation(selectedConversationId);
    try {
      ...
      onLoadConversation(messages, selectedConversationId);
    } catch (err) {
  ...
  return (
    <div className="view-chats-panel">
      ...
      {loading && !conversations.length && chatMode && <p className="view-chats-loading">Loading…</p>}
      {!chatMode && <p className="view-chats-empty">Open a chat topic from the URL to see previous chats.</p>}
