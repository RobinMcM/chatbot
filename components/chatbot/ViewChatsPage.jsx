...
export default function ViewChatsPage() {
  ...
  useEffect(() => {
    fetch('/api/chat-modes')
      ...
      .then((data) => {
        ...
        setChatModes(Array.isArray(data.chat_modes) ? data.chat_modes : []);
      })
  ...
  async function handleExpand(c) {
    ...
    try {
      ...
      if (data.error) setExpandedMessages([{ role: 'error', content: data.error }]);
      else setExpandedMessages(Array.isArray(data.messages) ? data.messages : []);
    } catch (err) {
  ...
  return (
    <div className="view-chats-page">
      <header className="view-chats-header">
        <Link href="/chatbot" className="view-chats-back">Back to home</Link>
      ...
      <div className="view-chats-filters">
        <label className="view-chats-label">
          ...
          <input className="view-chats-input" placeholder="Filter by client ID" value={filterClientId} onChange={(e) => setFilterClientId(e.target.value)} />
        </label>
        <label className="view-chats-label">
          <span className="view-chats-label-text">Email / Phone</span>
          <input className="view-chats-input" placeholder="Filter by email or phone" value={filterEmail} onChange={(e) => setFilterEmail(e.target.value)} />
        </label>
        <label className="view-chats-label">
          <span className="view-chats-label-text">Topic</span>
          <select className="view-chats-select" value={filterTopic} onChange={(e) => setFilterTopic(e.target.value)} disabled={!chatModes.length}>
            ...
            {chatModes.map((mode) => {
              ...
              return (
                <option key={id} value={id}>{typeof label === 'string' ? label : String(id ?? '')}</option>
              );
            })}
          </select>
        </label>
        <button type="button" className="view-chats-apply-btn" onClick={fetchConversations} disabled={loading}>
          {loading ? 'Loading…' : 'Apply'}
        </button>
      </div>

      {loading && !conversations.length && <p className="view-chats-loading">Loading…</p>}
      {!loading && conversations.length === 0 && <p className="view-chats-empty">No conversations match the filters.</p>}
          ...
          <table className="view-chats-grid">
            ...
            <tbody>
              {conversations.map((c) => {
                ...
                return (
                  <Fragment key={rowKey}>
                    <tr className={isExpanded ? 'view-chats-grid-row--expanded' : ''}>
                      <td>{c.chat_mode || '—'}</td>
                      ...
                      <td>
                        <button type="button" className="view-chats-grid-cell-preview" onClick={() => handleExpand(c)} aria-expanded={isExpanded}>{preview}</button>
                      </td>
                      <td><time dateTime={c.created_at}>{c.created_at ? new Date(c.created_at).toLocaleString() : '—'}</time></td>
                    </tr>
                    {isExpanded && (
                          ...
                          <div className="view-chats-grid-expanded-inner">
                            <div className="view-chats-grid-expanded-header">
                              <div className="view-chats-grid-expanded-header-left">
                                ...
                                <button type="button" className="view-chats-format-toggle" onClick={() => setViewAsFormatted((v) => !v)} aria-pressed={viewAsFormatted}>
                                  {viewAsFormatted ? 'Text' : 'Formatted'}
                                </button>
                                {expandedMessages.length > 0 && (
                                  <span className="view-chats-grid-total-cost">
                                    Total cost: {formatCost(expandedMessages.filter((m) => m.role === 'assistant' && m.usage).reduce((sum, m) => sum + (getMessageCost(m.usage) ?? 0), 0))}
                                  </span>
                                )}
                              </div>
                              <button type="button" className="view-chats-delete-btn" onClick={() => handleDeleteConversation(c)} disabled={deletingKey !== null}>
                            ...
                            {loadingMessages === rowKey ? (
                              ...
                              expandedMessages.map((msg, j) => (
                                <div key={j} className={`view-chats-grid-message view-chats-grid-message--${msg.role === 'user' ? 'user' : 'assistant'}`}>
