...
export function parseTemplateMeta(content) {
  ...
  return { displayName, promptInfo, rulesOnly };
}
...
export function listChatModes() {
  ...
  for (const name of fs.readdirSync(RULES_DIR)) {
    ...
    if (ALLOWED_EXTENSIONS.includes(ext)) names.add(path.basename(name, ext));
  }
  return [...names].sort();
}
...
export function loadRulesWithFallback(chatMode) {
  const normalized = typeof chatMode === 'string' ? chatMode.trim() : '';
  const direct = normalized ? loadRules(normalized) : null;
  if (direct) return { ruleId: normalized, loaded: direct, usedFallback: false };

  const fallback = loadRules(DEFAULT_RULE_ID);
  if (fallback) return { ruleId: DEFAULT_RULE_ID, loaded: fallback, usedFallback: true };
